<template>
  <div class="flex flex-row px-4 py-1 mb-2 border-2 rounded-lg self-center justify-center align-center bg-white">
    <button @click="dropdownOpen = !dropdownOpen" class="flex relative z-10 block rounded-md bg-white p-2 focus:outline-none">
      ...
      <div v-show="dropdownOpen" class="absolute mt-12 py-2 w-48 bg-white rounded-md shadow-xl z-20 border-2">
        <div class="block px-4 text-sm capitalize text-gray-700 hover:bg-blue-500 hover:text-white">
          Settings
        </div>
        <hr class="my-2">
        <a @click="logout" href="#" class="block px-4 text-sm capitalize text-gray-700 hover:bg-blue-500 hover:text-white">
          Log out
        </a>
      </div>
    </div>
